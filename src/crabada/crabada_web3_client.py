...
class CrabadaWeb3Client(SwimmerNetworkClient):
    ...
    contract_address = T.cast(
        Address, "0x9ab9e81Be39b73de3CCd9408862b1Fc6D2144d2B"
    )
    this_dir = os.path.dirname(os.path.realpath(__file__))
    abi_dir = os.path.join(
        os.path.dirname(this_dir), "web3_utils", "abi", "abi-crabada.json"
    )
    ...
    def start_game(self, team_id: int) -> HexStr:
        ...
        tx: TxParams = self.build_contract_transaction(
            self.contract.functions.startGame(team_id)
        )
        return self.sign_and_send_transaction(tx)

    def attack(
        self, game_id: int, team_id: int, expired_time: int, certificate: int
    ) -> HexStr:
        """
        Attack an open mine
        """
        tx: TxParams = self.build_contract_transaction(
            self.contract.functions.attack(
                game_id, team_id, expired_time, certificate
            )
        )
        return self.sign_and_send_transaction(tx)
    ...
    def close_game(self, game_id: int) -> HexStr:
        ...
        """
        tx: TxParams = self.build_contract_transaction(
            self.contract.functions.closeGame(game_id)
        )
        return self.sign_and_send_transaction(tx)
    ...
    def settle_game(self, game_id: int) -> HexStr:
        ...
        """
        tx: TxParams = self.build_contract_transaction(
            self.contract.functions.settleGame(game_id)
        )
        return self.sign_and_send_transaction(tx)

    def reinforce_defense(
        self, game_id: int, crabadaId: int, borrow_price: Wei
    ) -> HexStr:
        """
        ...
        tx: TxParams = self.build_contract_transaction(
            self.contract.functions.reinforceDefense(
                game_id, crabadaId, borrow_price
            ),
