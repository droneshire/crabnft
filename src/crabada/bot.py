...
class CrabadaBot:
    ...
    def __init__(self, user : str, config : UserConfig, dry_run : bool) -> None:
        ...
        self.crabada_w3 = T.cast(CrabadaWeb3Client, (CrabadaWeb3Client()
    ...
    def _print_mine_status(self) -> None:
        ...
        for mine in open_mines:
            logger.print_normal(f"\t{mine['game_id']}\t\tround {mine['round']}\t\t{self.crabada_w2.get_remaining_time_formatted(mine)}")
        logger.print_normal("\n")
    ...
    def _check_and_maybe_reinforce_mines(self) -> None:
        ...
        for team in teams:
            ...
            affordable_crabs = [ c for c in available_crabada if wei_to_tus(c["price"]) < self.config["max_reinforcement_price_tus"]]
            ...
            logger.print_normal(f"Found reinforcement crabada {crabada_id} for {price_tus} Tus [BP {battle_points} | MP {mine_points}]")

            tx_hash = self.crabada_w3.reinforce_defense(team["game_id"], reinforment_crabada["crabada_id"], reinforment_crabada["price"])
            tx_receipt = self.crabada_w3.getTransactionReceipt(tx_hash)
            if tx_receipt["status"] != 1:
