...
class MechBot:
    ...
    async def try_to_mint(
        ...
        time_unit_str = "hours" if time_window > 60.0 * 60.0 else "minutes"
        logger.print_bold(
            f"We've minted {nft_type}s {num_mints} in past {time_window} {time_unit_str} out of {len(mints_within_past_period)} mints"
        )
